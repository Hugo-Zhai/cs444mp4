...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        key_dim = self.query_dim
        # Similarity scores: (batch_size, num_tokens, num_tokens)
        similarity = torch.matmul(Q, K.transpose(-2, -1)) / math.sqrt(key_dim)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x_out = x_norm * self.w.view(1, 1, -1) + self.b.view(1, 1, -1)
